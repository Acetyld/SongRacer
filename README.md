...
- live preview scrub row also shows sampled last-frame index span (`returned_last_sample_frame_index/total_last_sample_frame_index`),
...
- preview API includes sampled last-frame indices (`returned_last_sample_frame_index`, `total_last_sample_frame_index`),
...
- source-duration metadata (`source_duration_seconds`) remains identical between fresh/cache-hit responses,
...
- builder UI surfaces capability limits fetched from backend contract (preview fps/frames/world-height, generator count, safe risk/attempts, safe analysis height).
...
- racer crop centers and sync fields are clamped to config-safe bounds on load/sync/submit (`crop in [0,1]`, `trim in [0,30]`, `offset in [-30,30]`).
